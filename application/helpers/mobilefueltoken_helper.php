...
function get_token($karyawanid, $ip, $penggunaid)
{
    ...
    $query = $ci->db->get_where("hrd.tokenmobile", ["karyawanid" => $karyawanid, "penggunaid" => $penggunaid]);
    $data_token = [
        "mytoken" => $token_bersih,
        "karyawanid" => $karyawanid,
        "penggunaid" => $penggunaid,
        "ip" => $ip
    ];
    if ($query->num_rows() > 0) {
        $ci->db->where('penggunaid', $penggunaid)->update("hrd.tokenmobile", $data_token);
    } else {
        $ci->db->insert("hrd.tokenmobile", $data_token);
    }
